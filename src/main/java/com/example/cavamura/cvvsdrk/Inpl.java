...
public class Inpl {
    ...
    public Inpl(Context context) {
        banco = new Database(context);
        c = context;
    }

    public Inpl(String canac, String nome){
        this.canac = canac;
        this.nome = nome;
    }
    ...
    public Cursor getInpls(){
        Cursor cursor;
        String[] campos =  {"_id","canac","nome"};
        db = banco.getReadableDatabase();
        cursor = db.query("INPLS", campos, null, null, null, null, null, null);
        if(cursor!=null){
            cursor.moveToFirst();
        }
        db.close();
    ...
    public Inpl getInpl(String id){
        try{
            Cursor cursor;
            String[] campos =  {"_id","canac","nome"};
            String where = "canac = " + id;
            db = banco.getReadableDatabase();
            cursor = db.query("INPLS", campos, where, null, null, null, null, null);
            if(cursor!=null){
                cursor.moveToFirst();
    ...
    public ArrayList<String> getTurmas(){
        ArrayList<String> inpls = new ArrayList<String>();
        Cursor cursor = this.getInpls();
    ...
    public boolean inserir() {
        try {
            ...
            resultado = db.insert("inpls", null, valores);
            db.close();
            if (resultado == -1) {
    ...
    public boolean remover(String id){
        try {
            ...
            String where = "canac =" + id;
            int resultado = db.delete("inpls",where,null);
            db.close();
            if (resultado == -1) {
